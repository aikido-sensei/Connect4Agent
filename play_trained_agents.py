...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Play moveshist agent against config_{i} agents.")
    parser.add_argument("--progress", type=bool, default=False, help="Show board after each move")
    args = parser.parse_args()

    for i in range(4):
        print(f"\n==== MATCH: moveshist (Player 1) vs config_{i} (Player 2) ====\n")

        # Player 1: moveshist agent
        moveshist_params = Hyperparameters(0)  # Use any valid config ID (0–3) to avoid init_agent() breaking
        moveshist_params.model_path = "models/moves_hist/model_latest_moveshist.pth"

        # Player 2: config_{i} agent
        config_params = Hyperparameters(i)

        evaluate(moveshist_params, config_params)

        print(f"\n==== MATCH: config_{i} (Player 1) vs moveshist (Player 2) ====\n")

        evaluate(config_params, moveshist_params)
